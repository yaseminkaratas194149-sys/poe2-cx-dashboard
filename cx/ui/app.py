...
status ring sweeps the live stage progress, then refreshes the view. ↻ relaunches,
✕ quits. Single-instance; Ctrl+Q quits.
"""
import json
import os
...
class CxApp(tk.Tk):
    ...
    def _build_toolbar(self):
        ...
        o_run, self.btn_run = seg_cell(seg, "▷ Run cycle", width=96, primary=True,
                                       font=("Segoe UI", 9))
    ...
    def _run_cycle(self):
        ...
        self._running = True
        self._cycle_error = False
        self.btn_run._active = True
        self.btn_run.config(text="…", bg=self.btn_run._hover_bg)
        self.lbl.config(text="pulling…", fg=FG_DIM)
    ...
    def _after_cycle(self, err):
        self._running = False
        self._poll_job = None
        self.btn_run._active = False
        self.btn_run.config(text="▷ Run cycle", bg=self.btn_run._base_bg)
        self.ring.reset()
